...
//            Map<String, Object> pfmap = patentFeatureWordMapper.selectByPid(patentid);
            ...
            if (checktype.equals(PatentSearchConstant.autosearch)){

                if (null == corelist || corelist.size()==0){
                    JsonModel resjson = new JsonModel(true, "暂无检索结果数据","200",null);
                ...
                wordparam.put("corewords",clist);
                wordparam.put("featurewords",tlist);

                searchparam.put("term",wordparam);
                searchparam.put("ipc",category);

                RestTemplate rest = new RestTemplate();
                logger.info("☆☆☆☆☆☆☆☆【check method autosearch 】  patent  【term2doc】   api param :"+searchparam);
                JSONObject jres = rest.postForEntity(patent_term2doc_api, searchparam, JSONObject.class).getBody();

                List<String> pids = (List<String>) jres.get("Pids");
                ...
                SearchResponse search = ESUtil.searchByPids(restHighLevelClient,pids, index, type);
...
//                    String appid = (String) sourceAsMap.get("appid");//申请号
                ...
                allwords.addAll(clist);
                allwords.addAll(tlist);
                resultmap.put("words",allwords);//返回抽取的特征词全集
...
//            String resdata = HttpClientUtils.JsonPostInvoke(patent_term2doc_api, searchparam);
//            JSONObject json = JSONObject.parseObject(resdata);
            ...
            int m = patentSearchHistoryMapper.insertSelective(patentSearchHistory);
//            {corewords=[饮料, 杯, 充气, 过滤], featurewords=[]}
            if (null == pids){
                JsonModel resjson = new JsonModel(true, "暂无检索结果信息","200",null);
                return JSON.toJSONString(resjson);
            }
...
//            SearchResponse search = ESUtil.searchByPids(restHighLevelClient,new ArrayList<>(set), index, type);
            SearchHits hits = search.getHits();
            long total = hits.totalHits;
